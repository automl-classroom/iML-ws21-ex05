...
class StyledPlot:
    ...
    def subplots(self, cols=1, rows=1, **kwargs):
        return self.plt.subplots(nrows=rows, ncols=cols, figsize=(FIG_WIDTH*cols, FIG_HEIGHT*rows), dpi=200, **kwargs)
    ...
    def boxplot(self, values, positions, color, widths=0.5):
        bp = self.plt.boxplot(values, positions=positions,
        ...
        for flier in bp['fliers']:
            flier.set(marker='o', markersize=3, markerfacecolor=color,
                      linestyle='none', markeredgecolor='none', color=color, alpha=0.5)
